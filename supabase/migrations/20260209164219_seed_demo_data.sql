...
BEGIN
  ...
  -- Create ~150 purchases with variable distribution (creating ups and downs)
  -- Use a weighted distribution to simulate busy days and slow days
  FOR i IN 1..150 LOOP
    rand_photo := photo_ids[1 + floor(random() * array_length(photo_ids, 1))::int];
    rand_cust := cust_ids[1 + floor(random() * 40)::int];

    -- Create variable day distribution: some days get many purchases, some get few
    -- Using a non-uniform distribution based on day patterns
    DECLARE
      days_ago integer;
      day_weight float;
      random_val float;
    BEGIN
      -- Generate weighted days: weekends and certain "event" days are busier
      random_val := random();

      -- Create clusters: high activity periods and low activity periods
      IF random_val < 0.15 THEN
        -- 15% on very busy days (2-3 days ago - recent spike)
        days_ago := 2 + floor(random() * 2)::int;
      ELSIF random_val < 0.30 THEN
        -- 15% on busy weekend (6-8 days ago)
        days_ago := 6 + floor(random() * 3)::int;
      ELSIF random_val < 0.42 THEN
        -- 12% on promotion day (14-15 days ago)
        days_ago := 14 + floor(random() * 2)::int;
      ELSIF random_val < 0.54 THEN
        -- 12% on busy weekend (20-22 days ago)
        days_ago := 20 + floor(random() * 3)::int;
      ELSIF random_val < 0.64 THEN
        -- 10% on event day (27-28 days ago)
        days_ago := 27 + floor(random() * 2)::int;
      ELSE
        -- 36% spread across other days (creating valleys)
      ...
      VALUES (
        ...
        CASE
          WHEN random() > 0.94 THEN 'refunded'
